...
> 上游：[产品需求](../10-product/product-requirements.md)、[系统设计](system-design.md)、[证据模型](evidence-model.md)、[扫描与分析](scanning-and-analysis.md)、[ADR-0002](../30-decisions/adrs/ADR-0002-python-and-offline-typescript-dashboard.md)、[ADR-0003](../30-decisions/adrs/ADR-0003-evidence-pointers-without-source-snapshots.md)、[ADR-0004](../30-decisions/adrs/ADR-0004-dynamic-role-lens.md)  
> 下游：[验收基线](../40-delivery/acceptance-baseline.md)

## 1. 产物边界与输入

一次岗位准备围绕一个主岗位进行。它冻结一个 `PreparationRun`：已终态的 `ScanRun`、一个动态 `RoleLens`、所选 `ClaimRevision`、`ProjectContextFact`、覆盖状态、知识缺口和中文主语言。`PreparationRun`、`ArtifactSnapshot`、`DerivedExport`、`InterviewReview` 的字段与状态以[证据模型](evidence-model.md)为唯一事实源。

- Owner 通过显式 Skill 会话提供工作区、主岗位、可选 JD、可选职级覆盖和可选英文导出请求；无 JD 不阻断流程，RoleLens 必须把假设显示在产物中（`FR-01`、`FR-02`、`FR-05`）。
- 首版主包语言固定为中文。英文简历和英文问答从已成功的中文主快照按需派生，不触发重新扫描，也不得引入新事实（`FR-13`）。
- 只有引用至少一个可用 `ProjectSnapshot` 的终态扫描运行可以开始岗位准备。`partial` 扫描允许生成带缺口的 `partial` 准备包；`failed` 扫描不能发布新 `ArtifactSnapshot`（`FR-15`、`NFR-05`）。
...
每个 snapshot manifest 至少引用：`preparation_run_id`、报告契约版本、canonical `report_bundle_sha256`、RoleLens、所用 ScanRun/ProjectSnapshot disposition、Claim/Evidence 版本、覆盖与问题摘要、InterviewReview ID/截点、语言和生成器版本。它不包含源码全文、完整面试对话或 SQLite 内部表结构。

### 5.2 英文按需导出

默认只生成中文主包。Owner 明确请求英文时，从指定 `ArtifactSnapshot` 创建新的不可变 `DerivedExport`，仅包含英文简历和英文问答；技术名、Evidence ID、状态、数字和角色事实必须保持语义一致。导出不重新扫描、不读取数据库当前态替换冻结内容、不生成英文 HTML，也不修改源快照或 `latest`。没有中文源材料或指定快照不存在时，英文导出失败且不留下半成品（`FR-13`）。
...
一轮模拟结束后，只追加 `InterviewReview`：题目 ID、结构化摘要、掌握等级、薄弱点和可选 `next_review_at`。不保存完整逐字对话、音频或未结构化回答，也不创建定时任务、通知或主动提醒。看板只显示已记录的复习状态和日期，由 Owner 自主决定何时再次练习（`FR-14`、`NFR-06`）。
...
| `ART-08` | 指定中文快照并请求英文 | 新的不可变 DerivedExport，包含同一事实源的英文简历和英文问答 | 不生成英文 HTML；成功或失败均不改源快照/latest；不重新扫描或补造事实 | `FR-13`、`NFR-04` |
