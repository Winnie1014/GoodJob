...
- 本表是决策状态索引，不复制 ADR 的完整论证；待接受项不是已接受架构。
- 决策按“已接受”“待接受”“未来扩展”“已否决”分区；待接受项不得作为已接受架构实施。修改已接受决策必须新增 ADR 或替代原 ADR。
