...
| 序 | 任务 | 卡面 | 状态 | 体量上限 | 验收项 |
| --- | --- | --- | --- | --- | --- |
| 1 | GJ-11 · 给文档门禁补测试并按覆盖收敛 | [GJ-11](../collab/tasks/GJ-11.md) | 实施中（Sol；#21 派发 → #23 L1 → #24 裁决 → #25 L1 → #26 裁决放行） | 320（删除行不计） | `SCAN-04` |
| 2 | GJ-06 · 剥离 Git 元数据与沙箱调用 | [GJ-06](../collab/tasks/GJ-06.md) | 已派发（信道 #21），**须在 GJ-11 合入后开工** | 净新增 200（移动行单列） | `SCAN-04` |
