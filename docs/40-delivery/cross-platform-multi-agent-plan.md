...
> 当前状态（2026-08-14）：Phase 3-A 已终审并接受 ADR-0011；Phase 3-B 运行时当前仅为 PR #3 的未合入候选实现；`IMP-31A-G` 真机准入尚未通过，因此原生 Windows 继续 unsupported / fail-closed 并推荐 WSL2。下文保留派工时的 spike 与实现步骤作为交付追溯，不表示这些步骤已经获得发布证据。
