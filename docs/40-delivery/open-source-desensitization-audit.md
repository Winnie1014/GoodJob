...
| 维度 | 结论 | 覆盖范围 | 关键结果 |
| --- | --- | --- | --- |
| 密钥/凭据 | ⚪ | 149 个 commit、476 个唯一 blob | 敏感文件名、私钥、云 token、提交消息秘密均为零；2 个通用赋值形状均为类型化令牌误报 |
...
脚本 SHA-256 分别为 `28999d31e0a968a081b0b540deb23ac7ded04b1b646beea95d9eacf23bdfa133` 与 `454fc22ed3883c1b1bcbe93c9764e1e09e651c272d6591b1a236754bc7f84d2f`。两份脚本的正、负校准均通过。除脚本输出外，本报告还用 `git log -S`、逐 commit `git grep`、`git ls-tree` 与 `git cat-file` 做定点复核。
...
- 唯一的 2 个通用赋值形状位于 `.agents/skills/goodjob-career-review/runtime/src/goodjob/exporting.py` 的历史行 235/269，内容语义是 `InlineToken.from_value` 产生的局部令牌对象，不是认证凭据；两处均复核为误报。
...
文件内容独立扫描得到 250 个邮箱形状：168 个示例地址、49 个 agent no-reply 地址、33 个 Owner 已批准身份的历史 blob 命中；未分类邮箱为 0。手机号形状为 0。author 与 committer 元数据在 149/149 个 commit 中均为同一项 Owner 已批准身份；提交消息中该身份出现 1 次（`261d96c`，消息第 3 行）。

为避免在公开报告中复制身份原文，具体文件定位只列路径与行号：

- 当前 `docs/40-delivery/backlog.md:306`、`docs/collab/tasks/GJ-22.md:47`、`docs/collab/channel.md:3407,3701,3751` 含该已批准身份的披露。
- Owner 本机目录段的当前实例仅在 `docs/collab/channel.md:2267,2291,2454,3321`、`docs/collab/protocol.md:204`、`docs/collab/implementer-bootstrap.md:10`、`docs/collab/tasks/GJ-16B.md:15`、`docs/collab/tasks/GJ-18.md:31`；这些属于 #117 精确列举的协作记录白名单。
...
- 当前非 `docs/` 路径没有 `CodeRoute` 或 `SliverShield` 命中；历史命中全部位于协作文档，另有一处测试 fixture 的合成包名。
...
固定主体的 `git rev-list --all --not 5af1413 --count` 为 0；相对 `main` 为 5，全部来自当前 GJ-22 分支的裁决记录提交。其余 branch/ref 没有 `main` 独有内容。完整 `git branch -a` 输出如下：
...
下列常见 repo-local 路径目前未被提交的根规则覆盖：`runtime/build/`、`src/goodjob.egg-info/`、`goodjob.sqlite3`、`artifacts/`、`exports/`、`drafts/`、`.env`、`config.toml`。本地 `runtime/dist/` 目前靠生成目录内的未跟踪 `.gitignore` 隔离，fresh clone 不会得到这层保护；任意 `--data-dir` 也无法靠固定 `.gitignore` 覆盖。

结论：🟡 建议级。开源前应提交窄范围规则并在干净 clone 中用 `git check-ignore` 验证；运行时数据继续放在仓库外，不把任意用户目录写入仓库。

## 4. 独立复核、零原文与限制

独立结果与派卡时 Architect 的机械结论在“无秘密、无未授权 PII、无真实外部源码”三个方向一致。数字变化来自审计主体从早期 120/147 个 commit 增长到本次固定的 149 个 commit，以及按唯一 blob 去重的统计口径；没有出现新的未授权类别。#118/#119 的 acceptance-baseline 历史区间已重新计入。

交付前对本报告和本次新增信道消息做了类别扫描：没有私钥片段、认证 token、电话号码、Owner 身份原文或本机真实目录段；报告只保留白名单引用的 commit/path/line。扫描范围与结果可由 `git diff --check`、敏感模式扫描和人工逐段复核重现。
