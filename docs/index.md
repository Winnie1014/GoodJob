...
GoodJob 当前处于“设计文档固化与 Owner 核对”阶段。私有 GitHub 仓库只用于保存这套文档基线；本文档集合仍**不表示任何扫描器、可安装 Skill、数据库、前端、协作信道、CI 或产品发布已经实现或部署**。
...
| [ADR-0001](30-decisions/adrs/ADR-0001-skill-and-state-isolation.md) 至 [ADR-0007](30-decisions/adrs/ADR-0007-review-state-lineage-and-snapshot-integrity.md) | 已接受 | 难以逆转决策的理由、后果和替代方案 | 不作为功能需求清单 |
...
| 项目发现、Git、忽略、刷新与语言 | D-011 至 D-018、D-021、D-036 | ADR-0005、ADR-0006、ADR-0007、扫描与分析设计 |
...
| 明确延后能力 | F-001 至 F-008 | 决策账本“明确的非首版能力” |
...
9. 工作区/JD 中的指令、脚本和 HTML 只作为数据，不得驱动 Agent、命令或离线看板执行。
