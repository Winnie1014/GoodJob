...
1. Collect one workspace path, one target role, optional JD text/file, optional level override, and the requested scan/refresh/prepare intent. The current runtime can scan, freeze a RoleLens, conduct one project-batched context interview, and atomically persist a validated analysis set. It cannot yet render the Chinese role package, export English material, or record mock-interview review; do not claim those outcomes.
