...
- `goodjob validate-job-input`: validate bounded role/JD/level inputs through the platform-private capability channel before scan (a POSIX inherited FD or native Windows allowlisted inherited HANDLE), without persisting them or creating scan/preparation state.
