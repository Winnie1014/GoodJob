...
1. Collect one workspace path, one target role, optional JD text/file, optional level override, and the requested scan/refresh/prepare intent. The current runtime can scan, freeze a RoleLens, conduct one project-batched context interview, atomically persist a validated analysis set, and render an immutable Chinese Markdown/offline-HTML role package. It cannot yet export English material or record mock-interview review; do not claim those outcomes.
...
- `goodjob interview`: append exactly one complete context answer/partial/skip batch, create reusable context facts and user-statement Evidence, and return the run to `analyzing`.
...
- `scripts/session.py`: task-scoped JSONL broker for authorization, scan overview/scan/refresh, preparation, context, source verification, atomic analysis, rendering, external Git metadata, and targeted history operations; it exits when input closes and retains no capability, active PreparationRun binding, or history-candidate approval afterwards. Rendering is deliberately task-independent because it consumes only a frozen owner-local analysis.
